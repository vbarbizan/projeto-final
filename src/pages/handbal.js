import React from "react";
import { Link } from "react-router-dom";
import Sidebar from "../components/Sidebar";
import handbal from "../assets/handbal.png"
import logo from "../assets/logo.png"
import Medalha from ".././components/medalhas/Medalha"
import './hand.css';
import SemiFinal from "../components/chave-grupo/chavedoVolei";



function Handebol() {
  return (
    <>
      <Sidebar/>

      <div className="container">
        <img src={handbal} className="imgVolei" />
        <h1>HANDBALL</h1>
        <p>Transforme a atividade física em um vício, que ele dará habilidades para vencer outros</p>

      </div>

      <div className="tabela">
        <img src={logo} />
        <h1>InterSESI</h1>

      </div>
      
      <div className="tabelas">
        <div className="pai">
          <div className="title">
            <h2>Classficação</h2>
            <h3>Interclasse - 2024</h3>

          </div>
          <div className="medalha">
            <Medalha />

          </div>

        </div>
        <SemiFinal className="semi" />
      </div>

    </>
  );
}

export default Handebol;